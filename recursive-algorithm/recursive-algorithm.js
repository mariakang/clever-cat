...
function runTests() {
  ...
  if (avgsOnly) {
    ...
    let divisorsArr = dataPointsArr.filter(obj => obj.x >= maxN - 5).map(obj => Math.pow(3, obj.x * 2 / 3) / obj.y);
    let divisor = Math.round(divisorsArr.reduce((x, y) => x + y) / divisorsArr.length);
    let compName = "1/" + divisor + " * 3 ^ (2/3 * n)";
    let compColor = color.replace("50%)", "85%)");
    let compDataPointsArr = testSizes.map(function (n) {
      return {
        x: n,
        y: round(Math.pow(3, n * 2 / 3) / divisor)
      };
    });
